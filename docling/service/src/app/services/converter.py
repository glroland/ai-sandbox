...
from docling.datamodel.base_models import ImageRefMode, InputFormat
...
from app.config import Device, OcrEngine, settings
...
_OCR_OPTIONS_MAP: dict[OcrEngine, Any] = {
    OcrEngine.rapidocr: RapidOcrOptions(),
    OcrEngine.easyocr: EasyOcrOptions(),
    OcrEngine.tesseract: TesseractCliOcrOptions(),
}
...
def _get_converter(page_batch_size: int) -> DocumentConverter:
    ...
    ocr_options = _OCR_OPTIONS_MAP[settings.ocr_engine]
    ...
    log.info(
        "Initialising DocumentConverter — device=%s ocr_engine=%s page_batch_size=%d",
