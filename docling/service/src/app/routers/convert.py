...
async def convert_document(
    ...
    """
    log.info("POST /convert/ filename='%s' batch_size=%d", filename, page_batch_size)
...
@router.post(
    "/text",
    status_code=status.HTTP_200_OK,
    summary="Alias for POST /convert/ — stream plain Markdown from a binary upload",
    response_description="text/plain chunked stream, one page per chunk; images base64-embedded",
)
async def convert_document_to_text(
    ...
    """Convenience alias for ``POST /convert/``.

    The request body must be the raw binary file content
    (``Content-Type: application/octet-stream``).
    """
    log.info("POST /convert/text filename='%s' batch_size=%d", filename, page_batch_size)
    _check_extension(filename)
