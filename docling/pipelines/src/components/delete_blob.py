...
def delete_blob_op(
    ...
    """
    Permanently delete *blob_name* from *container_name*.

    Parameters
    ----------
    container_name  : container that holds the blob
    ...
    headers = _build_headers("DELETE", container_name, blob=blob_name)

    print(f"[INFO] Deleting '{blob_name}' from '{container_name}'")
    resp = requests.delete(url, headers=headers)
    if not resp.ok:
        print(f"[ERROR] delete_blob failed: {resp.status_code} {resp.text}")
        resp.raise_for_status()

    print(f"[INFO] Delete complete: '{blob_name}'")
